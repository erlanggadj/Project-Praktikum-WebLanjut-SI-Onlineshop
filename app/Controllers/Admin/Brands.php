...
class Brands extends BaseController
{
    ...
    public function index($brandId = null)
    {
        if ($brandId) {
            ...
            if (!$brand) {
                session()->setFlashdata('errors', 'Invalid brand');
    ...
    public function store()
    {
        ...
        if ($this->brandModel->save($params)) {
            session()->setFlashdata('success', 'Brand has been saved.');
            return redirect()->to('/admin/brands');
        } else {
            $this->getBrands();
    ...
    public function update($id)
    {
		...
		if ($this->brandModel->save($params)) {
			session()->setFlashdata('success', 'Brand has been updated!');
			return redirect()->to('/admin/brands');
		} else {
			$this->getBrands();
			$this->data['errors'] = $this->brandModel->errors();
    ...
    public function destroy($id)
    {
		...
		if ($this->brandModel->delete($brand->id)) {
			session()->setFlashdata('success', 'The brand has been deleted');
			return redirect()->to('/admin/brands');
		} else {
			session()->setFlashdata('errors', 'Could not delete the brand');
			return redirect()->to('/admin/brands');
		}
    }
}