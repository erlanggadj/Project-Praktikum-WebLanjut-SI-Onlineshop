...
class Attributes extends BaseController
{
    ...
    public function index($attributeId = null)
    {
        if ($attributeId) {
            ...
            if (!$attribute) {
                session()->setFlashdata('errors', 'Invalid attribute');
    ...
    public function store()
    {
        ...
        if ($this->attributeModel->save($params)) {
            session()->setFlashdata('success', 'attribute has been saved.');
            return redirect()->to('/admin/attributes');
        } else {
            $this->getAttributes();
    ...
    public function update($id)
    {
		...
		if ($this->attributeModel->save($params)) {
			session()->setFlashdata('success', 'attribute has been updated!');
			return redirect()->to('/admin/attributes');
		} else {
			$this->getAttributes();
			$this->data['errors'] = $this->attributeModel->errors();
    ...
    public function destroy($id)
    {
		...
		if ($this->attributeModel->delete($attribute->id)) {
			session()->setFlashdata('success', 'The attribute has been deleted');
			return redirect()->to('/admin/attributes');
		} else {
			session()->setFlashdata('errors', 'Could not delete the attribute');
			return redirect()->to('/admin/attributes');
		}
    }
}
